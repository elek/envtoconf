...
func ParseKey(declaration string, envs map[string]string) (filename string, format string, key string, err error) {
	...
	if len(parts) > 1 {
		...
		} else if definedFormat, ok := envs[strings.ToUpper(name)+"."+strings.ToUpper(extension)]; ok {
			format = strings.ToLower(definedFormat)
		} else {
			format = extension
		}
...
func ParseKeyValues(envs map[string]string) map[string]Configfile {
	...
	for key, value := range envs {
		filename, format, key, err := ParseKey(key, envs)
